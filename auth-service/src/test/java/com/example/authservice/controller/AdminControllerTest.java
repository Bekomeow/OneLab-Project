...
class AdminControllerTest {
    ...
    void deleteUser_ShouldReturnSuccessMessage() throws Exception {
        ...
        doNothing().when(adminService).deleteUser(username, reason);

        mockMvc.perform(MockMvcRequestBuilders.post("/auth/admin/delete-user/{username}", username)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(new ObjectMapper().writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(content().string("Пользователь " + username + " удален."));

        verify(adminService, times(1)).deleteUser(username, reason);
    }
